...
class ImageCompressorNode(Node):
    def __init__(self):
        super().__init__('image_compressor_left')

        # Inicializar el puente de OpenCV
        self.bridge = CvBridge()

        # Suscripción a la imagen izquierda
        self.sub = self.create_subscription(Image, '/stereo_virtual/left/image_raw', self.image_callback, 10)

        # Publicador de imagen comprimida
        self.pub = self.create_publisher( CompressedImage, '/stereo_virtual/left/out/compressed', 10)

        # Publicar 15 Hz
        self.last_publish_time = time.time()
        self.publish_interval = 1.0 / 15.0

        self.get_logger().info("ImageCompressorNode (left) iniciado.")
